...
bool MusicTestScene::handle_input(uil::Context const& context) const {
    ...
    for (const auto& [rectangle,func] : m_buttons) {
        auto const col = Rectangle{ rectangle.x * static_cast<float>(context.resolution.x), rectangle.y * static_cast<float>(context.resolution.y),
                                    rectangle.width * static_cast<float>(context.resolution.x), rectangle.height * static_cast<float>(context.resolution.y) };
...
void MusicTestScene::render(const uil::Context& context) const {
    ...
    for (auto const& [button, label] : std::views::zip(m_buttons, labels)) {
        ...
        auto const col  = Rectangle{ rect.x * static_cast<float>(context.resolution.x), rect.y * static_cast<float>(context.resolution.y),
                                    rect.width * static_cast<float>(context.resolution.x), rect.height * static_cast<float>(context.resolution.y) };
